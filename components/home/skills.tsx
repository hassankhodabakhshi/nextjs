...
const SkillsSection = () => {
  ...
  const renderSectionTitle = (): React.ReactNode => (
    <div className="flex flex-col">
      <p className="section-title-sm seq">مهارت‌ها</p>
      <h1 className="section-heading seq mt-2">مهارت‌های من</h1>
      <h2 className="text-2xl md:max-w-2xl w-full seq mt-2">
      من علاقه‌مندم مسئولیت طراحی یک تجربه کاربری زیبا را با استفاده از معماری فرانت‌اند مدرن بر عهده بگیرم.{" "}
      </h2>
    </div>
  );
    ...
    <section className="relative">
      ...
      >
        <div className="flex flex-col skills-wrapper">
          {renderSectionTitle()}
          <div className="mt-10">
            {renderSkillColumn("توسعه فرانت‌اند", SKILLS.frontend)}
          </div>
          <div className="flex flex-wrap mt-10">
            <div className="mr-6 mb-6">
              {renderSkillColumn(
                "طراحی رابط کاربری، طراحی تجربه کاربری",
                SKILLS.userInterface
              )}
            </div>
            <div>{renderSkillColumn("سایر مهارت‌ها", SKILLS.other)}</div>
          </div>
        </div>
      </div>
    </section>
